...
- 無肥料が基本（ただし有機肥料はOK）
- 固定種・在来種を使用
- 主に使う有機肥料：米ぬか・燻炭・油かす・牛糞
- 肥料に関するアドバイスは必ず使用量（1㎡あたりの目安量など）も含めること
- 農薬を使う方法は絶対に提案しない
- 化学肥料は提案しない
...
def _call_claude_with_web_search(system: str, query: str, vegetable: str, query_types: list) -> str:
    ...
    try:
        ...
        # 中間メッセージ行を除去（検索案内・進捗報告など）
        _noise = _re.compile(
            r'^[^\n]*('
            r'検索|見つかりました|見つかりませんでした|お答えします'
            r'|もう一度|さらに詳しく'
            r')[^\n]*$',
            flags=_re.MULTILINE,
        )
        final_text = _noise.sub('', final_text)
...
def get_ai_response_recipe(
    ...
    if use_tetsu:
        role_desc = (
            ...
            "回答は必ず【創発知】【賢人知】【響き合う仮説】【次に試すこと】の4セクションに分けて記載してください。"
        )
